        ...
        <div class="content-section">
                            ...
                            <tr>
                                <td>
                                    @if(!empty($item['imagen']))
                                        @if(Str::startsWith($item['imagen'], 'http'))
                                            <img src="{{ $item['imagen'] }}" class="cart-img" alt="{{ $item['nombre'] }}">
                                        @elseif(file_exists(public_path('storage/'.$item['imagen'])))
                                            <img src="{{ asset('storage/'.$item['imagen']) }}" class="cart-img" alt="{{ $item['nombre'] }}">
                                        @elseif(file_exists(public_path('img/'.$item['imagen'])))
                                            <img src="{{ asset('img/'.$item['imagen']) }}" class="cart-img" alt="{{ $item['nombre'] }}">
                                        @else
                                            <img src="{{ asset('img/Captura de pantalla 2025-03-16 190807.png') }}" class="cart-img" alt="Producto sin imagen">
                                        @endif
                                    @else
                                        <img src="{{ asset('img/fisherprice.jpg') }}" class="cart-img" alt="Producto sin imagen">
                                    @endif
                                </td>
                                <td>{{ $item['nombre'] ?? 'Producto sin nombre' }}</td>
                                <td>${{ number_format($item['precio'] ?? 0, 2) }}</td>
                                <td>
                                    <form action="{{ route('carrito.actualizar', $id_producto) }}" method="POST" class="quantity-control">
                                        @csrf
                                ...
                                <td>
                                    <form action="{{ route('carrito.eliminar', $id_producto) }}" method="POST">
                                        @csrf
            ...
            @else
                <div class="empty-cart">
                    <p>Tu carrito está vacío. <a href="{{ url('Galeria') }}">Ver productos</a></p>
                </div>
            @endif
        </div>
    </div>
